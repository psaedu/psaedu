...
# Example of drawing daily load curve of active power
...
month = 1
day = 1 
# Get the load data of specific day
...
#ax.step(load[0,:]+0.25,load[1,:], linewidth = 1,color='k')  #You can also draw step load curve  
a = np.argmin(load[1,:])
b = np.argmax(load[1,:])
ax.set_xlim([0, 25])
ax.set_ylim([0,5000])
...
ax.set_title(str(year)+'/'+str(month)+'/'+str(day)+' Load Curve')
plt.show()
